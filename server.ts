...
function floorToInterval(ts: Date, intervalMinutes: number) {
  const date = new Date(ts);
  const minutes = date.getMinutes();
  const floored = Math.floor(minutes / intervalMinutes) * intervalMinutes;
  date.setMinutes(floored, 0, 0);
  return date;
}

function aggregateToInterval(data: Array<{ timestamp: string; net_kw: number }>, intervalMinutes: number) {
  const buckets = new Map<number, { timestamp: string; net_kw: number }>();
  data.forEach((item) => {
    const ts = new Date(item.timestamp);
    if (isNaN(ts.getTime())) return;
    const bucketTime = floorToInterval(ts, intervalMinutes).getTime();
    const existing = buckets.get(bucketTime);
    if (!existing || item.net_kw > existing.net_kw) {
      buckets.set(bucketTime, { timestamp: new Date(bucketTime).toISOString(), net_kw: item.net_kw });
    }
  });
...
async function startServer() {
  ...
  app.post("/api/analyze", upload.array("files"), async (req: any, res) => {
    ...
    try {
      ...
      const aggregatedData = aggregateToInterval(uniqueData, 30);
      res.json({ 
        data: aggregatedData,
        metadata: {
  ...
  app.post("/api/simulate", (req, res) => {
    ...
    const results = data.map((row: any, i: number) => {
      ...
      const nonEssentialShareClamped = Math.max(0, Math.min(1, nonEssentialShare));
      const nonEssentialKw = nonHvacKw;
      const switchedOffNonEssentialKw = isPeakWindow ? nonEssentialKw * nonEssentialShareClamped : 0;
      const effectiveNonHvacKw = Math.max(0, nonHvacKw - switchedOffNonEssentialKw);
      
      // Solar generation
      const solarKw = calculateSolarGeneration(ts, solarCapacity);
      
      // Net load before battery
      let netLoadKw = baselineKw - solarKw - switchedOffNonEssentialKw;
      ...
      const optimizedKw = Math.max(0, effectiveNonHvacKw + hvacKw - solarKw + batteryPower);
      ...
      return {
        ...
        switched_off_non_essential_kw: switchedOffNonEssentialKw,
        net_load: netLoadKw
      };
    });
